 ...
 *        cd contracts && USDC_ADDRESS=<arc-testnet-usdc> forge script script/Deploy.s.sol \
 *          --rpc-url $ARC_TESTNET_RPC_URL --broadcast --private-key $DEPLOYER_PRIVATE_KEY
 *      then record addresses in deployments/arc-testnet.json.
 *   2. Buyer + seller wallets funded with testnet USDC from the Circle faucet
 ...
 *
 *  Env for this script: BUYER_PRIVATE_KEY, optionally SELLER_URL. */

const MICRO_CALLS = Number(process.env.MICRO_CALLS ?? 10);
